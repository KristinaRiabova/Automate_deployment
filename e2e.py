...
class TestAdditionalFunctionality(unittest.TestCase):
 def test_get_reports(self):
        ...
        for report_info in data:
            self.assertIsInstance(report_info, dict)
            self.assertIn('name', report_info)
            self.assertIn('metrics', report_info)
            self.assertIn('users', report_info)

 def test_get_user_average_time_invalid_metric(self):
     user_id = '02d4563d-5727-c811-b3b7-57a10f6be25a'
     invalid_metric_type = 'invalid_metric'
     response = requests.get(f'{BASE_URL}/api/stats/user/average?userId={user_id}&metricType={invalid_metric_type}')
